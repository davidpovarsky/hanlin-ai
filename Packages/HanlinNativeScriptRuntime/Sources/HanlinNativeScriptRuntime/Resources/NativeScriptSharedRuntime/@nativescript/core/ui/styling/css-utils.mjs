...
function hasNestedParens(value, open) {
    while (open !== -1) {
...
function splitTopLevel(value, commaIsSeparator) {
    const parts = [];
    let depth = 0;
    let start = 0;
 ...
 */
export function splitOnTopLevelWhiteSpace(value) {
 ...
 */
export function splitOnTopLevelSpacesAndCommas(value) {
    if (!value.includes('(')) {
 ...
 */
export function splitOnTopLevelComma(value) {
    const open = value.indexOf('(');
    if (open === -1) {
        return value.split(',');
    }
...
export function parseCSSCommaSeparatedListOfValues(value) {
    const values = [];
    if (!value) {
        return [];
    }
