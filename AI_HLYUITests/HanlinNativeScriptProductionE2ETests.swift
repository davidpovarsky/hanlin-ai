...
final class HanlinNativeScriptProductionE2ETests: XCTestCase {
    ...
    func testWholeAppPrimaryNavigationSmoke() throws {
        let tabs = [
            ...
            app.tabBars.buttons["hanlin-settings-tab"],
            app.tabBars.buttons["hanlin-apps-tab"]
        ]
        ...
        XCTAssertTrue(app.tabBars.buttons["hanlin-apps-tab"].isSelected || app.tabBars.buttons["hanlin-apps-tab"].exists)
    }
    
    func testReEntrancyAndDuplicateTapProtection() throws {
        openApps()
        let addButton = appsAddButton
    ...
    func testCommandLevelProductionAcceptance() throws {
        let settingsTab = app.tabBars.buttons["hanlin-settings-tab"]
        if settingsTab.waitForExistence(timeout: 5) {
            settingsTab.tap()
        }
        
        let runtimeCenter = app.buttons["hanlin-runtimes-packages-link"].firstMatch
        if runtimeCenter.waitForExistence(timeout: 5) {
            runtimeCenter.tap()
            
            let smokeShell = app.buttons["hanlin-runtime-smoke-shell"].firstMatch
            if smokeShell.waitForExistence(timeout: 5) {
                smokeShell.tap()
                let lastMessage = app.staticTexts["hanlin-runtime-last-message"].firstMatch
                XCTAssertTrue(lastMessage.waitForExistence(timeout: 10))
                XCTAssertFalse(lastMessage.label.isEmpty)
            }
        }
    }
}
