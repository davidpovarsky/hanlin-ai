...
const manifest = {
  ...
  runtimes: [
    ...
    },
    {
      ...
      linkDependencyHashes: Object.fromEntries(lock.iosSystem.linkDependencies.map(dependency => [dependency.name, dependency.sha256])),
      linkDependencies: lock.iosSystem.linkDependencies.map(dependency => ({
        name: dependency.name,
        role: 'application-link-dependency',
        sourceRepository: dependency.sourceRepository,
        release: dependency.release,
        archiveHash: dependency.sha256,
        license: dependency.license,
        distribution: 'resolved-and-embedded-during-application-build',
      })),
