...
struct HanlinCanonicalToolBackendTarget: Hashable, Sendable {
  ...
  let toolPresentation: HanlinToolPresentationDescriptor
}
...
struct HanlinCanonicalToolBackendRouteIndex: Sendable {
  ...
  private let targetsByLogicalID: [HanlinLogicalToolID: HanlinCanonicalToolBackendTarget]
  ...
  init(
    ...
    where record.target.backend.providerIdentity
...
struct HanlinCanonicalToolAuthority {
  ...
  func resolution(alias: String) -> Resolution? {
    ...
    return Resolution(
      ...
      toolPresentation: target.toolPresentation
    )
  }

  func toolDescriptor(for alias: String) -> HanlinToolDescriptor? {
    guard let route = routingTable.route(alias: alias) else { return nil }
    return catalog.entries.first(where: { $0.descriptor.logicalID == route.logicalToolID })?
      .descriptor
  }
  ...
  static func build(
    ...
    for source in nativeSources.sorted(by: { $0.entry.name < $1.entry.name }) {
      ...
      var profile = source.entry.presentationProfile
      if profile.canonicalExecutionPresentation == nil {
        profile.canonicalExecutionPresentation =
          projection.descriptor.presentation.executionPresentation
      }
      if profile.canonicalEmbeddedPresentation == nil {
        profile.canonicalEmbeddedPresentation =
          projection.descriptor.presentation.embeddedPresentation
      }
      if profile.result == nil && projection.descriptor.presentation.embeddedPresentation != nil {
        profile.result = ToolResultPresentationDescriptor(
          rendererKind: .modernNative, supportsCard: true)
        if profile.resultDisplayPolicy == .never {
          profile.resultDisplayPolicy = .modelControlled
        }
      }
      candidates.append(
        Candidate(
    ...
    for tool in mcpTools {
      ...
      var profile = MCPToolBridge.presentationProfile(descriptor: tool)
      if profile.canonicalExecutionPresentation == nil {
        profile.canonicalExecutionPresentation = descriptor.presentation.executionPresentation
      }
      if profile.canonicalEmbeddedPresentation == nil {
        profile.canonicalEmbeddedPresentation = descriptor.presentation.embeddedPresentation
      }
      if profile.result == nil && descriptor.presentation.embeddedPresentation != nil {
        profile.result = ToolResultPresentationDescriptor(
          rendererKind: .modernNative, supportsCard: true)
        if profile.resultDisplayPolicy == .never {
          profile.resultDisplayPolicy = .modelControlled
    ...
    for source in scriptSources {
      ...
      if profile.canonicalExecutionPresentation == nil {
        profile.canonicalExecutionPresentation =
          source.descriptor.presentation.executionPresentation
      }
      if profile.canonicalEmbeddedPresentation == nil {
        profile.canonicalEmbeddedPresentation = source.descriptor.presentation.embeddedPresentation
      }
      if profile.result == nil && source.descriptor.presentation.embeddedPresentation != nil {
        profile.result = ToolResultPresentationDescriptor(
          rendererKind: .modernNative, supportsCard: true)
        if profile.resultDisplayPolicy == .never {
          profile.resultDisplayPolicy = .modelControlled
      ...
      records: resolved.map {
        HanlinCanonicalToolBackendRecord(
          ...
          target: HanlinCanonicalToolBackendTarget(
            ...
            toolPresentation: $0.candidate.descriptor.presentation
          )
        )
      }
    )
    guard routes.count == entries.count,
      Set(routes.map(\.logicalToolID))
        == Set(entries.map { $0.descriptor.logicalID })
    else {
      throw HanlinCanonicalToolAuthorityError.routeInvariant(
  ...
  private struct Candidate {
    ...
    static func precedes(_ left: Self, _ right: Self) -> Bool {
      ...
      let leftIdentity =
        "\(left.descriptor.logicalID.providerInstanceID.rawValue)"
        + "|\(left.descriptor.logicalID.localToolID.rawValue)"
      let rightIdentity =
        "\(right.descriptor.logicalID.providerInstanceID.rawValue)"
