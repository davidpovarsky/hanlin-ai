...
enum AgentActivityComposer {
  ...
  private static func matchingGroupIndex(
    for step: AgentActivityStep, in groups: [[AgentActivityStep]]
  ) -> Int? {
    guard step.kind == .toolCall || step.kind == .toolExecution || step.kind == .webSearch else {
      return nil
    }
    ...
    guard let tool = AgentActivityDeduplicator.normalized(step.subtitle),
      let index = groups.lastIndex(where: { group in
        guard let last = group.last,
          abs(last.startedAt.timeIntervalSince(step.startedAt)) < 8
        else { return false }
  ...
  private static func makeActivity(_ steps: [AgentActivityStep]) -> AgentDisplayActivity? {
    ...
    case .reasoning:
      title = first.title
    default:
      title = AgentActivityTitleBuilder.title(
        for: steps, kind: kind, queries: queries, status: status)
    }
    guard ProgressSummarySanitizer.sanitize(title) != nil || kind != .narrative else { return nil }

    let executionPresentation =
      steps.compactMap(\.canonicalExecutionPresentation).first
      ?? steps.compactMap(\.presentationProfile?.canonicalExecutionPresentation).first
    let embeddedPresentation =
      steps.compactMap(\.canonicalEmbeddedPresentation).first
      ?? steps.compactMap(\.presentationProfile?.canonicalEmbeddedPresentation).first
  ...
  private static func mergeAdjacentSearches(_ activities: [AgentDisplayActivity])
    -> [AgentDisplayActivity]
  {
    ...
    for activity in activities {
      guard activity.kind == .search,
        ...
        abs(currentStart.timeIntervalSince(previousStart)) < 20
      else {
        result.append(activity)
        continue
      }
      result.removeLast()
      previous.queries = AgentActivityDeduplicator.uniqueStrings(
        previous.queries + activity.queries)
      previous.searchProviderName = previous.searchProviderName ?? activity.searchProviderName
      previous.sources = AgentActivityDeduplicator.uniqueSources(
        previous.sources + activity.sources)
      ...
      previous.title = AgentActivityTitleBuilder.title(
        for: [], kind: .search, queries: previous.queries, status: previous.status)
  ...
  private static func combinedDisplayStatus(_ lhs: AgentActivityStatus, _ rhs: AgentActivityStatus)
    -> AgentActivityStatus
  {
  ...
  private static func removeNarrativeDuplicates(_ activities: [AgentDisplayActivity])
    -> [AgentDisplayActivity]
  {
    var result: [AgentDisplayActivity] = []
    ...
    for (index, activity) in activities.enumerated() {
      if activity.kind == .narrative {
        guard let key = AgentActivityDeduplicator.normalized(activity.narrativeText),
          !seenNarratives.contains(key)
        else { continue }
        let nextToolTitle = activities.dropFirst(index + 1).first(where: { $0.kind != .narrative })?
          .title
  ...
  private static func summaryTitle(for run: AgentRun, duration: TimeInterval?) -> String {
    ...
    case .completed:
      return String(
        format: String(localized: "Worked for %@"),
        AgentActivityDurationFormatter.string(duration ?? run.elapsedTime))
    case .failed:
      return String(
        format: String(localized: "Stopped after %@"),
        AgentActivityDurationFormatter.string(duration ?? run.elapsedTime))
    case .cancelled:
      return String(
        format: String(localized: "Cancelled after %@"),
        AgentActivityDurationFormatter.string(duration ?? run.elapsedTime))
    }
  }
}
