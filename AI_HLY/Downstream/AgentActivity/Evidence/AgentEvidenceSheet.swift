...
struct AgentEvidenceSheet: View {
    ...
    private var groups: [EvidenceGroup] {
        let visibleItems = items.filter(\.wasReturnedToModel)
        return EvidenceSection.all.compactMap { section in
            let values = visibleItems.filter { section.kinds.contains($0.kind) }
            return values.isEmpty ? nil : EvidenceGroup(
                id: section.id,
                title: section.title,
                items: values
            )
...
private struct EvidenceGroup: Identifiable {
    var id: String
    var title: String
...
private struct EvidenceSection {
    ...
    static var all: [EvidenceSection] {
        [
            ...
            EvidenceSection(id: "other", title: String(localized: "Other items"), kinds: [.genericItem])
        ]
    }
}
