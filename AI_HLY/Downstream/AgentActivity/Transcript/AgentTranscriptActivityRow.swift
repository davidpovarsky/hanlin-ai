...
struct AgentTranscriptActivityRow: View {
  ...
  var body: some View {
    let explicitExecution = activity?.executionPresentation ?? item.canonicalExecutionPresentation
    let family = ChatPresentationBridge.executionFamily(
      explicit: explicitExecution,
      for: activity?.kind,
...
enum AgentActivityTrace {
  ...
  static func inspectorScrolled(runID: UUID, selectedActivityID: String) {
    log(
      "agent_inspector_scrolled_to_activity", runID: runID, selectedActivityID: selectedActivityID)
  }
