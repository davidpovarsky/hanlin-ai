...
struct AgentTranscriptAccumulator {
  ...
  init(items: [AgentTranscriptItem] = []) {
    ...
    for (index, item) in self.items.enumerated() {
      ...
      if item.kind == .userVisibleToolResult {
        resultDeduplicationKeys.insert(
          Self.resultKey(
            callID: item.callID,
  ...
  mutating func begin(
    ...
    if let index = itemIndexByExternalID[externalID], items.indices.contains(index) {
      if let toolName, items[index].toolName == nil {
        items[index].toolName = toolName
      }
      if let canonicalExecutionPresentation, items[index].canonicalExecutionPresentation == nil {
        items[index].canonicalExecutionPresentation = canonicalExecutionPresentation
      }
      if let canonicalEmbeddedPresentation, items[index].canonicalEmbeddedPresentation == nil {
        items[index].canonicalEmbeddedPresentation = canonicalEmbeddedPresentation
      }
    ...
    let item = AgentTranscriptItem(
      ...
      callID: callID,
      toolName: toolName,
      resultRendererKind: resultRendererKind,
  ...
  mutating func update(
    ...
    guard let index = itemIndexByExternalID[externalID], items.indices.contains(index) else {
      return
    }
  ...
  mutating func closeActiveItems(as status: AgentActivityStatus, completedAt: Date = Date()) {
    ...
    where items[index].status == .running || items[index].status == .pending {
      items[index].status = status
      items[index].completedAt = completedAt
