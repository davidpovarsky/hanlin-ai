...
enum ToolResultPresentationCoordinator {
  static func decide(
    ...
    let supportsCard = (profile.result?.supportsCard == true) || hasCanonicalEmbedded
    let displayPolicy =
      (profile.resultDisplayPolicy == .never && hasCanonicalEmbedded)
      ? .modelControlled
      : profile.resultDisplayPolicy
    let rendererKind = profile.result?.rendererKind ?? (hasCanonicalEmbedded ? .modernNative : nil)
  ...
  private static func suppressed(_ reason: ToolResultSuppressionReason)
    -> ToolResultPresentationDecision
  {
    ToolResultPresentationDecision(
      shouldPresent: false, rendererKind: nil, suppressionReason: reason)
  }
